...
class MultiHeadedSimilarity(nn.Module):
    def __init__(self, domain_dim, num_heads=5):
        super(MultiHeadedSimilarity, self).__init__()
        self.num_heads = num_heads
        # 更新权重维度以适应矩阵乘法 (num_heads, domain_dim, domain_dim)
        self.weights = nn.Parameter(torch.randn(num_heads, domain_dim))
        
    def forward(self, domain_embeddings):
        # domain_embeddings shape: (batch_size, num_domains, domain_dim)
        batch_size, num_domains, domain_dim = domain_embeddings.size()
        
        # 扩展权重以匹配 batch_size 和 num_domains
        # weights_expanded shape: (num_heads, 1, 1, domain_dim)
        weights_expanded = self.weights.unsqueeze(1).unsqueeze(1)
        
        # 扩展域嵌入以适应权重
        # expanded_embeddings shape: (1, batch_size, num_domains, domain_dim)
        expanded_embeddings = domain_embeddings.expand(self.num_heads,batch_size,num_domains, domain_dim)
        
        # 应用点乘权重并计算相似度
        # weighted_embeddings shape: (num_heads, batch_size, num_domains, domain_dim)
        weighted_embeddings = expanded_embeddings * weights_expanded
        
        # 计算所有头的余弦相似性
        # cosine_sim shape: (num_heads, batch_size, num_domains, num_domains)
        normalized_embeddings = F.normalize(weighted_embeddings, p=2, dim=3)
        cosine_sim = torch.einsum('hbid,hbjd-> hbij', normalized_embeddings, normalized_embeddings)
        
        # 平均所有头的相似性得到最终相似性矩阵
        # similarity_matrix shape: (batch_size, num_domains, num_domains)
        similarity_matrix = cosine_sim.mean(dim=0)
...
class earth_epi(nn.Module):
    def __init__(self, args, data): 
        ...
        edge_index, _ = dense_to_sparse(data.orig_adj)
        self.edge_index = edge_index.cuda()
        data.edge_index = edge_index.cuda()
        ...
        self.d_model = args.n_hidden
        args.d_model = self.d_model
        
        
        # self.multi_simi = MultiHeadedSimilarity(domain_dim=self.d_model, num_heads=5)
        ...
        # self.conv1 = GraphConvLayer(self.d_model, self.d_model)
        ...
        self.vector_field_f = FinalTanh_f(input_channels=self.input_dim , hidden_channels=args.n_hidden,
                                        hidden_hidden_channels=args.n_hidden,
                                        num_hidden_layers=1)
        self.vector_field_g = VectorField_g(input_channels=self.input_dim , hidden_channels=args.n_hidden,
                                        hidden_hidden_channels=args.n_hidden,
                                        num_hidden_layers=2, num_nodes=self.m, cheb_k=3, embed_dim=64,
                                        g_type="agc", adj = data.orig_adj)
        
        self.vector_field_g.orig_adj = data.orig_adj
        ...
        if self.init_type == 'fc':
            self.initial_h = torch.nn.Linear(self.input_dim, args.n_hidden)
            self.initial_z = torch.nn.Linear(self.input_dim, args.n_hidden)
            self.initial_s = torch.nn.Linear(self.input_dim, args.n_hidden)
            self.initial_i = torch.nn.Linear(self.input_dim, args.n_hidden)
            self.initial_r = torch.nn.Linear(self.input_dim, args.n_hidden)
        ...
        self.dtw = data.sem_mask
        edge_index, _ = dense_to_sparse(self.dtw)
        self.dtw_edge =  edge_index.cuda()  
        
        self.vector_field_g.dtw_edge = self.dtw_edge
        
        self.vector_field_g.dtw = data.sem_mask

        # 将A_1和A_2转换为布尔类型
        A_1_bool = data.orig_adj.bool()
        A_2_bool = data.sem_mask.bool()
        ...
        # 更新 A_1
        A_new = data.orig_adj + difference
        ...
        self.multi_view_fusion = MultiViewFusion(args.n_hidden, 4)
    ...
    def forward(self, times, coeffs, X):
        '''
        ...
        adjs  = self.adj.repeat(X.shape[0],1)
        adjs = adjs.view(X.shape[0], self.m, self.m)
        
        output_list = controldiffeq.cdeint_gde_dev(dX_dt=spline.derivative, #dh_dt
                                   h0= h0,
                                   z0= z0,
                                   s0 = s0,
                                   i0 = i0,
        ...
        # fused_feature = self.self_attetion(temporal_out[1:])
        ...
        # x_out = self.final_project(torch.cat([z_T,t_T],dim=-1))
        ...
        # # 666
        # # x = x.matmul(self.adj) 
        # orig_x = x #[batch, window, m]
        
        # new_x = x.permute(0, 2, 1).contiguous()
        
        
        # # new_x = x.permute(0, 2, 1).contiguous().reshape(B * M, 1, T)  #(B * M, 1, T)

    
        # new_x = self.embedding(new_x) #(B =, M, D)
        
    
        # # h_attn = self.graph_mamba(new_x)
        
        # # new_x = h_attn + new_x


        # x_out = self.SIR_mamba(new_x)


        # x_out = x_out + new_x
        
        # # x_embeddings = self.mamba_b1(x_out)
        
        # results = []
        
        # for i in range(self.m):
        #     # 提取第 i 个域的特征
        #     domain_features = x_out[:, i, :]  # (batch_size, domain_embedding_dim)
        #     # 通过第 i 个域的头部
        #     domain_output = self.proj_heads[i](domain_features)  # (batch_size, 1)
        #     # out = torch.squeeze(domain_output)

        #     results.append(domain_output)
            
        # outputs = torch.cat(results,dim=-1)
        

        # x_out = self.final_project(x_out)
        # outputs = torch.squeeze(x_out)
...
class cola_gnn_epi(nn.Module):
    def __init__(self, args, data): 
        ...
        self.GRU2 = nn.GRU(1, self.n_hidden, batch_first = True)
        ...
        self.GRU3 = nn.GRU(1, self.n_hidden, batch_first = True)
    ...
    def forward(self, x, feat=None):
        ...
        A = torch.Tensor(b, self.m, self.m)
        for batch in range(0, b):
            Sparse_adj_diagValue = torch.diag(torch.diagonal(Adj_Epi[batch]))
            W = torch.diag(torch.sum(Adj_Epi[batch],dim=0))-Sparse_adj_diagValue
            A[batch] = ((Adj_Epi[batch].T - Sparse_adj_diagValue) - W)
        ...
        # #sample * 1 * #location
        X_vector_t = orig_x[:,-1,:].view(b, 1, self.m)
        # transpose: #sample * #location * #location
        # NGMT = (NextGenerationMatrix ** self.h).permute(0,2,1)
        NGMT = (NextGenerationMatrix).permute(0,2,1).to(X_vector_t.device)
        outputNGMT = NextGenerationMatrix
        
        # print(X_vector_t.device)
        
        y_vector_t = X_vector_t.bmm(NGMT).view(b,-1)
        EpiOutput = y_vector_t

        x = r_l

        # ---- not softmax
        x = F.relu(self.conv1(x, adj_deep))

        x = F.dropout(x, self.dropout, training=self.training)

        # ---- not softmax
        out_spatial = F.relu(self.conv2(x, adj_deep))
