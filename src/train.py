...
ap.add_argument('--dataset', type=str, default='us_hhs', help="Dataset string")
ap.add_argument('--sim_mat', type=str, default='us_hhs-adj', help="adjacency matrix filename (*-adj.txt)")
...
ap.add_argument('--gpu', type=int, default=2,  help='choose gpu 0-10')
...
ap.add_argument('--patience', type=int, default=1000, help='patience default 100')
...
def evaluate(data_loader, data, modelName, tag='val'):
    ...
    for inputs in data_loader.get_batches(data, batch_size, False):
        ...
        if modelName == "cola_gnn_epi":
            ...
            results, dynamic_adj  = model(data_loader.times, train_coeffs, X)
        
        
        loss = F.l1_loss(results, Y)
        
        # loss_2 = relation_aware_consistency_loss(results, Y, dynamic_adj, F.l1_loss)
        # loss = _add_weight_regularisation(loss, model.vector_field_g) #TODO: regularization
        # loss = _add_weight_regularisation(loss, model.vector_field_f) #TODO: regularization
        ...
        # output = test_stage_prediction(results, dynamic_adj)
        output = results
...
def train(data_loader, data, modelName, Lambda):
    model.train()
    model.dtw = data_loader.sem_mask
    total_loss = 0.
    n_samples = 0.
    batch_size = args.batch
    ...
    for inputs in data_loader.get_batches(data, batch_size, True):
        ...
        model_parameters = model.parameters()  # Get model parameters
        l2_reg = sum(param.norm(2) for param in model_parameters)
        ...
        n_samples += (results.size(0) * data_loader.m)
